...
const galleryFiles = [
  ...
  "images/gallery/AUTO EXPO FINAL DRAFT.webp",
  "images/gallery/ESCAPE ROOM FINAL.webp",
  "images/gallery/FLAGSHIP EVENTS COVERPAGE (2).webp",
  "images/gallery/FLAGSHIP EVENTS COVERPAGE.webp",
] as const;
...
function HelixImage({
  ...
  useFrame(({ clock }) => {
    ...
    meshRef.current.rotation.x =
      wobbleBaseRotX.current + Math.sin(t * 1.1 + phaseOffset) * 0.02;
  ...
  const clampedProgress = Math.max(
    -visibleWindow,
    Math.min(visibleWindow, imageProgress),
  );
  ...
  const isVisible =
    imageProgress > -visibleWindow && imageProgress < visibleWindow;
...
export function SpiralGallery({ images = fallbackImages }: SpiralGalleryProps) {
  const [scrollProgress, setScrollProgress] = useState(0);
  const [isMobile, setIsMobile] = useState(false);
  const [isIntroComplete, setIsIntroComplete] = useState(false);
  const defaultImages = useMemo(() => {
    const localGalleryImages = galleryFiles.map(
      (file) => `/${file.split("/").map(encodeURIComponent).join("/")}`,
    );

    if (images.length > 0 && images[0] !== fallbackImages[0]) return images;
    if (localGalleryImages.length > 0) return localGalleryImages;
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(to bottom, #001428 0%, #002040 35%, #001a35 70%, #00101f 100%)",
        }}
      />
    </div>
  );
